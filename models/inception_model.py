...
class InceptionModel(object):
    ...
    @staticmethod
    def build_model(ckpt_path, align_tf=True):
        ...
        if align_tf:
            ...
            num_classes = 1000
            model_source = 'inception_v3_google'
        ...
        if fingerprint not in InceptionModel.models:
            ...
            url, hash_str = _MODEL_URL_SHA256[fingerprint]
            ...
            #     torch.hub.get_dir(), 'checkpoints', filename)
            file_path = ckpt_path
            with open(file_path, 'rb') as f:
                file_hash = hashlib.sha256(f.read())
                if file_hash.hexdigest() != hash_str:
                    warnings.filterwarnings('always', category=UserWarning)
                    warnings.warn(f'Hash check failed! The remote file from '
                                  f'url `{url}` may have been changed. The '
                                  f'loaded inception model may have strange '
                                  f'behavior.')
...
def build_inception_model(ckpt_path):
    return InceptionModel.build_model(ckpt_path=ckpt_path, align_tf=True)
